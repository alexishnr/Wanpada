  ...
  var _ref2 = _asyncToGenerator(function* (projectRoot, options = {}) {
    ...
    if (!hasIosDirectory && isIosSupported) {
      ...
      if (!exp.ios.bundleIdentifier) {
        console.log(`You'll need to specify an iOS bundle identifier. See: https://docs.expo.io/versions/latest/guides/configuration.html#bundleidentifier`);
    ...
    if (!hasAndroidDirectory) {
      ...
      if (!exp.android.package) {
        console.log(`You'll need to specify an Android package name. See: https://docs.expo.io/versions/latest/guides/configuration.html#package`);
    ...
    if (expoReactNativeTag && pkg.dependencies['react-native'] !== reactNativeVersion) {
      ...
      try {
        ...
        console.warn(`
        ${(_chalk || _load_chalk()).default.yellow('Unable to install the Expo fork of react-native.')}
        ${(_chalk || _load_chalk()).default.yellow(`Please install react-native@${reactNativeVersion} to complete detaching.`)}
      `);
        throw e;
      }
    }
